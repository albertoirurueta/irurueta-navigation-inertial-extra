...
def replace_dependency_snippets(text, release_version, next_snapshot):
    ...
    The marker line may be plain text ("Latest release:", as used in README.md) or an
    AsciiDoc heading ("== Latest release", as used in the Antora installation.adoc page) -
    leading "=" heading markers are stripped before the comparison.
    """
    lines = text.splitlines(keepends=True)
    pending = None
    for i, line in enumerate(lines):
        marker = line.strip().lstrip("=").strip().lower()
        if marker.startswith("latest release"):
            pending = release_version
        elif marker.startswith("latest snapshot"):
...
def update_pom(release_version, next_snapshot):
    ...
    updated, count = re.subn(
        r"(<artifactId>irurueta-navigation-inertial-extra</artifactId>\s*\n\s*<version>)[^<]*(</version>)",
        rf"\g<1>{next_snapshot}\g<2>",
        text,
        count=1,
    )
    if count != 1:
        sys.exit("pom.xml: could not find the irurueta-navigation-inertial-extra <version> element to bump")
    path.write_text(updated)


def update_readme(release_version, next_snapshot):
    # README.md's "Usage" section uses plain "Latest release:" / "Latest snapshot:" markers
    # ahead of each dependency snippet - there is no "Current development version" status table
    # row in this repository's README to bump separately.
