...
public abstract class AccelerometerAndGyroscopeIntervalDetectorThresholdFactorOptimizer extends
     ...
     */
    private AccelerometerAndGyroscopeMeasurementsGenerator generator;
     ...
     * MSE value.
     */
    private AccelerometerAndGyroscopeMseRule mseRule =
    ...
    protected double evaluateForThresholdFactor(final double thresholdFactor) throws LockedException,
        ...
        if (measurements.size() < accelerometerCalibrator.getMinimumRequiredMeasurements()
                || sequences.size() < gyroscopeCalibrator.getMinimumRequiredMeasurementsOrSequences()) {
            return Double.MAX_VALUE;
        }
            ...
            case STANDARD_DEVIATION_BODY_KINEMATICS:
                ...
                if (accelerometerCalibrator.isQualityScoresRequired()) {
                    ...
                    for (var i = 0; i < size; i++) {
                        qualityScores[i] = accelerometerQualityScoreMapper.map(measurements.get(i));
                    }
        ...
        if (gyroscopeCalibrator.isQualityScoresRequired()) {
            ...
            final var qualityScores = new double[size];
            for (var i = 0; i < size; i++) {
                qualityScores[i] = gyroscopeQualityScoreMapper.map(sequences.get(i));
            }
            calibrator.setQualityScores(qualityScores);
        }
        ...
        final var mse = mseRule.evaluate(accelerometerMse, gyroscopeMse);
        if (mse < minMse) {
    ...
    private void initialize() {
        final var generatorListener = new AccelerometerAndGyroscopeMeasurementsGeneratorListener() {
            @Override
